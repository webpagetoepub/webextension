...
For Firefox (`tests/firefox_extension_loads.spec.ts`, `tests/firefox_converts_page.spec.ts`):
- Playwright cannot load a Firefox extension, so the specs drive a real Firefox through **geckodriver** (the Marionette/WebDriver path, same as `web-ext run`). `tests/helpers/load_firefox_extension.ts` builds the extension, zips it into an `.xpi` with `fflate`, downloads the geckodriver binary via the `geckodriver` package, then `installAddon(xpi, /* temporary */ true)` installs it unsigned. The loads spec packages the shipped `dist/firefox`; the conversion spec packages a harness build (`buildExtension: () => buildTestExtension("firefox")`) with a pinned moz-extension UUID so it can navigate to `harness.html` and run the real bundled conversion in an extension page.
- `firefox_extension_loads.spec.ts` asserts the build *loads* as a temporary add-on (the resolved add-on id equals `browser_specific_settings.gecko.id`), guarding against manifest/bundle regressions.
- `firefox_converts_page.spec.ts` exercises conversion end to end and is a regression guard for the MV3 blob-worker hang (see `src/lib/fflate_main_thread.ts`). It pins **both** conditions needed to reproduce: a chapter ≥ 160 KB (fflate's Worker threshold) and the strict MV3 base CSP via the `extensions.webextensions.base-content-security-policy.v3` pref (newer Firefox relaxes the default and would mask the bug).
